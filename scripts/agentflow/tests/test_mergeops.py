...
def test_ci_green_requires_the_named_workflow_to_be_present():
    # I7: "a missing check is not green" was the one clause of the spec's CI
    # rule that had no implementation. A bot check that succeeds before
    # ci.yml's own jobs register must not read as green.
    unrelated = [{"workflowName": "Dependabot", "status": "COMPLETED", "conclusion": "SUCCESS"}]
    assert mergeops.ci_green(unrelated)  # unchanged when no workflow is required
    assert not mergeops.ci_green(unrelated, "CI")
    ours = [{"workflowName": "CI", "status": "COMPLETED", "conclusion": "SUCCESS"}]
    assert mergeops.ci_green(ours, "CI")
    assert mergeops.ci_green(ours + unrelated, "CI")
    assert not mergeops.ci_green(ours + [{"workflowName": "CI", "status": "COMPLETED",
                                          "conclusion": "FAILURE"}], "CI")
...
def test_merge_raises_a_named_error_when_github_has_no_merge_commit_yet(ctx):
    # An unpopulated `mergeCommit` used to be a bare TypeError at exit 2 --
    # a SUCCESSFUL merge surfacing to the conductor as a crash, which its stop
    # rule then reads as a failure while `master` already carries the commit.
    lock.acquire(ctx, 8, "s", 1)
    att = mergeops.Attestation(8, "b", "h", "f" * 40, "r", {}, "bh")
    r = FakeRunner({f"pr merge 12 --squash --match-head-commit {'f' * 40}": "",
                    "pr view 12 --repo SShadowS/al-sem --json mergeCommit": json.dumps({})})
    with pytest.raises(RuntimeError, match="merge-sha-unavailable"):
